  ...
  <div class="app">
    ...
    <div class="app__list">
      ...
      <div class="app__item--wrap">
        ...
        <list-item v-for='contact in filteredContacts' :key='contact.id' @delete-contact='deleteContact' @edit-contact='editContact' :contact='contact'>
        </list-item>
      </div>
    </div>
    <!--
...
export default {
  ...
  methods: {
    ...
    editContact(contact) {
      this.$refs.modalAdd.editContacts( contact);
    },
    addContact(contact) {
      ...
      if(edit_x){
        this.$notify({
          group: 'foo',
          title: `Контакт ${contact.name} успешно обновлен`
        });
      }else{
